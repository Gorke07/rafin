...
function parseSearchResults(html: string): BookLookupResult[] {
  ...
  $('div.product-cr').each((i, el) => {
    ...
    const author = $(el).find('div.author span a.alt span').text().trim()
      || $(el).find('div.author.compact.ellipsis a.alt').text().trim()
    const publisher = $(el).find('div.publisher span a.alt span').text().trim()
    const coverUrl = $(el).find('a.pr-img-link img').attr('src') || undefined
    ...
    if (productInfo) {
      ...
      if (parts[2]) pageCount = parseInt(parts[2]) || undefined
    }
...
export const kitapyurduScraper: BookScraper = {
  ...
  async lookup(isbn: string): Promise<BookLookupResult | null> {
    try {
      ...
      const productLink = $search('div.product-cr div.name.ellipsis a').first().attr('href')
      ...
      const coverUrl = $('div.pr_images img').first().attr('src')
        || $('img.js-jbox-book-cover').attr('src')
      ...
      $('div.pr_attributes div.attributes table tr, .attributes tr').each((_, row) => {
        ...
        } else if (label.includes('yayın tarihi') || label.includes('yayin tarihi') || label.includes('baskı')) {
          const year = value.match(/\d{4}/)
          if (year) publishedYear = parseInt(year[0])
      ...
      const description = $('#description_text span.info__text').text().trim()
        || $('span.info__text').text().trim() || undefined
