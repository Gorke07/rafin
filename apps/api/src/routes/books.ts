...
import { db, books, bookCategories, bookCollections, categories, collections, bindingTypes } from '@rafin/db'
import { eq, desc, like, or, isNull, and } from 'drizzle-orm'
...
export const bookRoutes = new Elysia({ prefix: '/api/books' })
  ...
  .post('/', async ({ body }) => {
    const bindingType = body.bindingType && bindingTypes.includes(body.bindingType as BindingType)
  ...
  .patch('/:id', async ({ params, body, set }) => {
    ...
    const { categoryIds, collectionIds, bindingType: bindingTypeInput, coverUrl: coverUrlInput, removeCover, ...bookData } = body

    const bindingType = bindingTypeInput && bindingTypes.includes(bindingTypeInput as BindingType)
