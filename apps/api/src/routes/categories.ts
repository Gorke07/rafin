    ...
    const categoriesWithCounts = await Promise.all(
      result.map(async (category) => {
        const bookCount = await db
          ...
          .where(and(eq(bookCategories.categoryId, category.id), isNull(books.deletedAt)))
  ...
  .get(
    ...
    async ({ params, set }) => {
      ...
      const categoryBooks = await db
        ...
        .from(bookCategories)
        .innerJoin(books, eq(bookCategories.bookId, books.id))
        .where(and(eq(bookCategories.categoryId, Number(params.id)), isNull(books.deletedAt)))
