...
export const bookNotesRoutes = new Elysia({ prefix: '/api/books' })
  ...
  .get('/:id/notes', async ({ params, request, set }) => {
    ...
    const notes = await db
      ...
      .where(
        and(
          eq(bookNotes.bookId, Number(params.id)),
  ...
  // Add note to a book
  .post('/:id/notes', async ({ params, body, request, set }) => {
    const session = await auth.api.getSession({ headers: request.headers })
  ...
  // Update a note
  .patch('/:id/notes/:noteId', async ({ params, body, request, set }) => {
    const session = await auth.api.getSession({ headers: request.headers })
    ...
    const existing = await db
      ...
      .where(
        and(
          eq(bookNotes.id, Number(params.noteId)),
          eq(bookNotes.userId, session.user.id)
        )
      )
      .limit(1)
  ...
  // Delete a note
  .delete('/:id/notes/:noteId', async ({ params, request, set }) => {
    const session = await auth.api.getSession({ headers: request.headers })
